...
fn print_bound_node_as_code_with_indent(node: &BoundNode, printer: DebugPrinter, buffer: &mut String) {
    match &node.kind {
        BoundNodeKind::ErrorExpression => print_bound_node_error_expression_as_code(printer, buffer),
        BoundNodeKind::Label(label) => print_bound_node_label_as_code(label, printer, buffer),
        BoundNodeKind::LabelReference(label_reference) => print_bound_node_label_reference_as_code(label_reference, printer, buffer),
        ...
        BoundNodeKind::FunctionCall(function_call) => print_bound_node_function_call_as_code(function_call, printer, buffer),
        BoundNodeKind::SystemCall(system_call) => print_bound_node_system_call_as_code(system_call, printer, buffer),
        BoundNodeKind::ArrayIndex(array_index) => print_bound_node_array_index_as_code(array_index, printer, buffer),
        BoundNodeKind::FieldAccess(field_access) => print_bound_node_field_access_as_code(field_access, printer, buffer),
        BoundNodeKind::BlockStatement(block_statement) => print_bound_node_block_statement_as_code(block_statement, printer, buffer),
        BoundNodeKind::IfStatement(if_statement) => print_bound_node_if_statement_as_code(if_statement, printer, buffer),
        BoundNodeKind::VariableDeclaration(variable_declaration) => print_bound_node_variable_declaration_as_code(variable_declaration, printer, buffer),
        BoundNodeKind::WhileStatement(while_statement) => print_bound_node_while_statement_as_code(while_statement, printer, buffer),
        BoundNodeKind::Assignment(assignment) => print_bound_node_assignment_as_code(assignment, printer, buffer),
        BoundNodeKind::ExpressionStatement(expression_statement) => print_bound_node_expression_statement_as_code(expression_statement, printer, buffer),
        BoundNodeKind::ReturnStatement(return_statement) => print_bound_node_return_statement_as_code(return_statement, printer, buffer),
        BoundNodeKind::Jump(jump) => print_bound_node_jump_as_code(jump, printer, buffer),
    }
}

fn print_bound_node_function_declaration_as_code(function_declaration: &BoundFunctionDeclarationNodeKind, printer: DebugPrinter, buffer: &mut String) {
    if function_declaration.is_main {
        buffer.push_str(&format!("fn main() "));
    } else {
...
fn print_bound_node_label_reference_as_code(label_reference: &usize, _: DebugPrinter, buffer: &mut String) {
    buffer.push_str(&format!("l#{}", label_reference));
}

fn print_bound_node_literal_expression_as_code(literal_expression: &LiteralNodeKind, _: DebugPrinter, buffer: &mut String) {
    buffer.push_str(&format!("{}", literal_expression.value));
}

fn print_bound_node_array_literal_expression_as_code(array_literal_expression: &BoundArrayLiteralNodeKind, printer: DebugPrinter, buffer: &mut String) {
...
fn print_bound_node_unary_expression_as_code(unary_expression: &BoundUnaryNodeKind, printer: DebugPrinter, buffer: &mut String) {
...
fn print_bound_node_binary_expression_as_code(binary_expression: &BoundBinaryNodeKind, printer: DebugPrinter, buffer: &mut String) {
...
fn print_bound_node_function_call_as_code(function_call: &BoundFunctionCallNodeKind, printer: DebugPrinter, buffer: &mut String) {
...
fn print_bound_node_system_call_as_code(system_call: &BoundSystemCallNodeKind, printer: DebugPrinter, buffer: &mut String) {
...
fn print_bound_node_array_index_as_code(array_index: &BoundArrayIndexNodeKind, printer: DebugPrinter, buffer: &mut String) {
...
fn print_bound_node_field_access_as_code(field_access: &BoundFieldAccessNodeKind, printer: DebugPrinter, buffer: &mut String) {
    print_bound_node_as_code_with_indent(&field_access.base, printer, buffer);
    buffer.push_str(&format!(".length"));
}

fn print_bound_node_block_statement_as_code(block_statement: &BoundBlockStatementNodeKind, mut printer: DebugPrinter, buffer: &mut String) {
    buffer.push_str("{\n");
    printer.indent += 4;
...
fn print_bound_node_if_statement_as_code(if_statement: &BoundIfStatementNodeKind, printer: DebugPrinter, buffer: &mut String) {
...
fn print_bound_node_variable_declaration_as_code(variable_declaration: &BoundVariableDeclarationNodeKind, printer: DebugPrinter, buffer: &mut String) {
...
fn print_bound_node_while_statement_as_code(while_statement: &BoundWhileStatementNodeKind, printer: DebugPrinter, buffer: &mut String) {
    buffer.push_str(&format!("while "));
...
fn print_bound_node_assignment_as_code(assignment: &BoundAssignmentNodeKind, printer: DebugPrinter, buffer: &mut String) {
...
fn print_bound_node_expression_statement_as_code(expression_statement: &BoundExpressionStatementNodeKind, printer: DebugPrinter, buffer: &mut String) {
    print_bound_node_as_code_with_indent(&expression_statement.expression, printer, buffer);
    buffer.push_str(";\n");
}

fn print_bound_node_return_statement_as_code(return_statement: &BoundReturnStatementNodeKind, printer: DebugPrinter, buffer: &mut String) {
...
fn print_bound_node_jump_as_code(jump: &BoundJumpNodeKind, printer: DebugPrinter, buffer: &mut String) {
    if let Some(condition) = &jump.condition {
        buffer.push_str(&format!("jump if "));
        print_bound_node_as_code_with_indent(&condition, printer, buffer);
        buffer.push_str(&format!(" is {} to  ", jump.jump_if_true));
    } else {
        buffer.push_str(&format!("jump to "));
    }
